    ...
    def make_output_human_readable(
        ...
        for prediction in predictions_list:
            if self.topn == 1:
                ...
                topk = torch.topk(torch.tensor(prediction), self.topn)
                label_str = '|'.join([self.vocab.get_token_from_index(label.item(), namespace=self.task) + '=' + str(score.item()) for label, score in zip(topk.indices, topk.values)])
                classes.append(label_str)
        return classes
