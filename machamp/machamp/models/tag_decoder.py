    ...
    def make_output_human_readable(
        ...
        for predictions in predictions_list:
            ...
            if self.topn == 1:
                ...
                tags = []
                for wordScores in predictions:
                    # TODO: why is it a numpy array in the first place?
                    topk = torch.topk(torch.tensor(wordScores), self.topn)
                    tags.append('|'.join([self.vocab.get_token_from_index(label.item(), namespace=self.task) + '=' + str(score.item()) for label, score in zip(topk.indices, topk.values)]))
                all_tags.append(tags)
        return all_tags
