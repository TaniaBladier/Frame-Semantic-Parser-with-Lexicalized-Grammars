...
class MachampBiaffineDependencyParser(Model):
    ...
    def forward(
        ...
        """
        predicted_heads, predicted_head_tags, mask, topn, arc_nll, tag_nll = self._parse(
    ...
    def make_output_human_readable(
        ...
        if self.topn not in [0, 1, None]:
            for instance_heads, instance_tags, length, topn_heads in zip(heads, head_tags, lengths, topnScores):
                ...
                topn_heads = topn_heads[1:length+1]
                topn_strings = []
                for i in range(0, length):
                    fullList = topn_heads[i].tolist()
                    if instance_heads[i] in fullList:
                        fullList.remove(instance_heads[i])
                    fullList = [instance_heads[i]] + fullList
                    if type(fullList[0]) == torch.Tensor:
                        fullList[0] = fullList[0].item()
                    topn_strings.append('|'.join([str(x) for x in fullList[:self.topn]]))
                head_tag_labels.append(labels)
                head_indices.append(topn_strings)
        else:
    ...
    def _parse(
        ...
        #)
        topnScores = None
        self.getTopN = not self.training and self.topn!=1
        if self.getTopN or (self.training or not self.use_mst_decoding_for_validation):
            predicted_heads, predicted_head_tags, topnScores = self._greedy_decode(
                head_tag_representation, child_tag_representation, attended_arcs, mask
            )
        else:
        ...
        return predicted_heads, predicted_head_tags, mask, topnScores, arc_nll, tag_nll
    ...
    def _greedy_decode(
        ...
        if self.getTopN:
            topk = torch.topk(attended_arcs, self.topn, dim=2)
            return heads, head_tags, topk.indices
        else:
            return heads, head_tags, None
